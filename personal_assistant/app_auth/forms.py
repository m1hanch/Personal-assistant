...
class RegisterForm(UserCreationForm):
    username = CharField(max_length=20, min_length=2, widget=TextInput(attrs={'class': 'form-control'}))
    email = EmailField(max_length=25, required=True, widget=EmailInput(attrs={'class': 'form-control'}))
    password1 = CharField(widget=PasswordInput(attrs={'class': 'form-control'}), required=True)
    password2 = CharField(widget=PasswordInput(attrs={'class': 'form-control'}), required=True)
